...
# --- Configuration ---
# User Agent diperbarui sesuai request terbaru
USER_AGENTS = [
...
def display_banner():
    ...
    console.print(f"[magenta]{'✔  BITQUANT AUTO BOT (UPDATED) ✔'.center(width)}[/magenta]")
    console.print(f"[yellow]{'Modified from the original script by NTExhaust'.center(width)}[/yellow]\n")
    console.print(f"[red]{'CATATAN PENTING: Website BitQuant telah menambahkan CAPTCHA.'.center(width)}[/red]")
    console.print(f"[red]{'Script ini kemungkinan besar akan GAGAL pada tahap pengiriman chat.'.center(width)}[/red]\n")
...
def get_base_headers(user_agent: str) -> dict:
    # Headers diperbarui agar lebih sesuai dengan browser asli
    return {
        'accept': '*/*',
...
async def get_id_token(client: httpx.AsyncClient, token: str) -> dict:
    ...
    # Headers x-client-version diperbarui
    headers = {
        'x-client-data': 'CIy2yQEIpbbJAQipncoBCNyHywEIlKHLAQiRo8sBCIWgzQEI4fDOAQil8s4BCJL2zgE=',
...
# --- [PERUBAHAN] Fungsi send_chat diperbarui ---
async def send_chat(client: httpx.AsyncClient, access_token: str, context: dict, message: str) -> dict:
    # Payload sekarang menyertakan captchaToken. Script ini mengirim placeholder, kemungkinan besar akan ditolak server.
    payload = {
        "context": context,
        "message": {"type": "user", "message": message},
        "captchaToken": f"0.placeholder.{base64.b64encode(os.urandom(64)).decode()}" # Placeholder, BUKAN token valid
    }
    headers = {'Authorization': f'Bearer {access_token}'}
    client.headers.update(headers)
    
    # Endpoint diubah ke v2
    response = await client.post('https://quant-api.opengradient.ai/api/v2/agent/run', json=payload)
    response.raise_for_status()
    return response.json() # Mengembalikan seluruh JSON response
...
async def process_accounts(private_keys, messages, account_proxies, chat_count, no_type):
    ...
    for i, private_key in enumerate(private_keys):
        ...
        conversation_history = [] # --- [PERUBAHAN] Riwayat chat disimpan di sini
        ...
        try:
            ...
            for j in range(chat_count):
                ...
                for attempt in range(max_retries_per_chat):
                    try:
                        async with httpx.AsyncClient(headers=session_state['base_headers'], timeout=45.0) as client:
                            if proxy:
                                client.proxies = {"http://": proxy, "https://": proxy}
                            ...
                            # --- [PERUBAHAN] Context sekarang menggunakan riwayat chat
                            context = { "conversationHistory": conversation_history, "address": address, "poolPositions": [], "availablePools": [] }
                            
                            response_data = await with_retry(lambda: send_chat(client, access_token, context, random_message), action_text="Sending Message")
                            
                            assistant_response = response_data.get('message', 'No response message found.')
                            await type_text(assistant_response, no_type)

                            # --- [PERUBAHAN] Simpan riwayat chat untuk request berikutnya
                        ...
                        console.print(f" [yellow]│ │ Attempt {attempt + 1}/{max_retries_per_chat} failed: {str(e)[:100]}. Retrying...[/yellow]")
                        if "captcha" in str(e).lower() or "403" in str(e):
                             console.print(f" [red]│ │ Gagal karena proteksi Captcha. Script tidak dapat melanjutkan untuk akun ini.[/red]")
                             daily_limit_reached = True # Anggap limit tercapai untuk skip ke akun lain
                             break
                        if attempt < max_retries_per_chat - 1:
                            await asyncio.sleep(15)
                        else:
                            console.print(f" [red]│ │ Semua percobaan gagal. Lanjut ke chat berikutnya.[/red]")
...
async def main():
    ...
    while True:
        ...
        await start_countdown(12 * 60 * 60)
